...
public class VDPCanvas extends Canvas {
    ...
    public void character(int col, int row, int[] pattern, int color) {
        pixelCharacter(col << 3, row << 3, pattern, color, true);
    }

    public void bitmap(int x, int y, int width, int height, int[][] patterns, int[][] colors) {
        for (int j = 0; j < height; j++) {
            for (int i = 0; i < width; i++) {
                int patternByte = patterns[j][i];
                int colorByte = colors[j][i];
                int mask = 0x80;
                for (int p = 0; p < 8; p++) {
                    plot(x + (i << 3) + p, y + j, (patternByte & mask) != 0 ? colorByte >> 4 : colorByte & 0x0F);
                    mask >>= 1;
                }
            }
        }
        ...
        for (int row = 0; row < 8; row++) {
            ...
            if (splash) {
                ...
                for (int x1 = (x & 0xF8); x1 < ((x & 0xF8) + ((x & 7) == 0 ? 8 : 16)); x1++) {
                    if (x1 >= 0 && y1 >= 0 && x1 < WIDTH && y1 < HEIGHT && buffer[y1][x1] != 0 && buffer[y1][x1] != color) {
                        plot(x1, y1, color);
                    }
                }
            }
